    ...
    if [ "$use_external_menu" = true ]; then
        case "$image_preview" in
            true)
                ...
                select_desktop_entry "" "Choose anime: "
                [ -z "$choice" ] && exit 1
    ...
    if [ "$use_external_menu" = true ]; then
        case "$image_preview" in
            true)
                ...
                select_desktop_entry "" "Choose manga: "
                [ -z "$choice" ] && exit 1
                media_id=$(printf "%s" "$choice" | cut -d\  -f1)
